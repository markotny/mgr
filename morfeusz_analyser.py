...
class MorfeuszAnalyser():
    ...
    def _analyse(self, text):
        analysis = [x for x in self.morf.analyse(text)
                    if x[2][1].split(':')[0] not in self.stop_words and x[2][2] not in self.ignore_tags]
        org_tokens = []
        lem_tokens = []
        curr_index = -1
    ...
    def __call__(self, text):
        if self.split_to_sentences:
            ...
            if self.use_multiprocessing:
                ...
                sentence_tokens = [self._analyse(sentence)
